...
const MCP_DEV_USER_ID = 'user_33Pe1G5JgFD0fw7eaKfFaRgzbJM'

app.get('/mcp/todos', async (c) => {
  const userTodos = await db.select()
    .from(todos)
    .where(eq(todos.userId, MCP_DEV_USER_ID))
  return c.json(userTodos)
})
...
app.patch('/mcp/todos/:id', async (c) => {
  ...
  const updatedTodo = await db.update(todos)
    .set(body)
    .where(and(eq(todos.id, id), eq(todos.userId, MCP_DEV_USER_ID)))
    .returning()
